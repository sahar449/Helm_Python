pipeline{
    ...
    stages{
        ...
        stage('Build Helm'){
            steps{
                script{
                    sh 'helm upgrade --install python helm --set image.tag="$BUILD_ID" '
                }
            }
        }
            ...
            steps{
                script{ 
                        withCredentials([string(credentialsId: 'nexus', variable: 'nexus')]) {
                            sh 'cd helm && helm package helm-$IMAGE_NAME:$BUILD_ID.tgz'
                            sh 'curl -u admin:$nexus minikube:30002/repository/helm/ --upload-file helm-$IMAGE_NAME:$BUILD_ID.tgz'
                        }
                    }
                }
            }
